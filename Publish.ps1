...
#   ShowPicOnly.exe                                    portable, Native AOT, runs without .NET installed
#   ShowPicOnly-FrameworkDependent-RequiresNET10.exe   single file, needs the .NET 10 Desktop Runtime
...
$project = Join-Path $PSScriptRoot 'ShowPicOnly.csproj'
...
Copy-Item "$workFolder\FrameworkDependent\ShowPicOnly.exe" "$publishFolder\ShowPicOnly-FrameworkDependent-RequiresNET10.exe" -Force
Copy-Item "$workFolder\Portable\ShowPicOnly.exe" "$publishFolder\ShowPicOnly.exe" -Force
